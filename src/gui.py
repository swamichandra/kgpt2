...
def ui_api_key():
    if ss['community_user']:
        ...
        with t1:
            ...
            st.write(
                f'Community tokens available: :{"green" if pct else "red"}[{int(pct)}%]')
            st.progress(pct/100)
...
def ui_pdf_file():
    ...
    with t1:
        st.file_uploader('pdf file', type='pdf', key='pdf_file', disabled=disabled,
